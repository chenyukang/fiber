...
For unlokcing the funds locked in current openning channels, we need to close all channels from old database, first we try to shutdown them cooperatively(which require the peer of channel online at the moment), then if not success, we continue try to shutdown channel forcely.
...
will return all channels(not includ closed channels):
