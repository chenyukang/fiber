...
impl FromStr for SignedRawBolt11Invoice {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if var == bech32::Variant::Bech32m {
            ...
            return Err(Bolt11ParseError::Bech32Error(
        ...
        Ok(SignedRawBolt11Invoice {
            ...
            hash: RawBolt11Invoice::hash_from_parts(hrp.as_bytes(), &data[..data.len() - 104]),
...
fn parse_tagged_parts(data: &[u5]) -> Result<Vec<RawTaggedField>, Bolt11ParseError> {
    ...
    while !data.is_empty() {
        ...
        match TaggedField::from_base32(field) {
            ...
            | Err(Bolt11ParseError::Bech32Error(bech32::Error::InvalidLength)) => {
...
impl FromBase32 for TaggedField {
    ...
    fn from_base32(field: &[u5]) -> Result<TaggedField, Bolt11ParseError> {
        ...
        match tag.to_u8() {
            ...
            constants::TAG_DESCRIPTION => Ok(TaggedField::Description(Description::from_base32(
                field_data,
            )?)),
            constants::TAG_PAYEE_PUB_KEY => Ok(TaggedField::PayeePubKey(PayeePubKey::from_base32(
                field_data,
            )?)),
            constants::TAG_DESCRIPTION_HASH => Ok(TaggedField::DescriptionHash(
                Sha256::from_base32(field_data)?,
            )),
            constants::TAG_EXPIRY_TIME => Ok(TaggedField::ExpiryTime(ExpiryTime::from_base32(
                field_data,
            )?)),
            ...
            constants::TAG_PRIVATE_ROUTE => Ok(TaggedField::PrivateRoute(
                PrivateRoute::from_base32(field_data)?,
            )),
            constants::TAG_PAYMENT_SECRET => Ok(TaggedField::PaymentSecret(
                PaymentSecret::from_base32(field_data)?,
            )),
            constants::TAG_PAYMENT_METADATA => Ok(TaggedField::PaymentMetadata(
                Vec::<u8>::from_base32(field_data)?,
            )),
            constants::TAG_FEATURES => Ok(TaggedField::Features(
                Bolt11InvoiceFeatures::from_base32(field_data)?,
            )),
            _ => {
...
impl FromBase32 for Sha256 {
    ...
    fn from_base32(field_data: &[u5]) -> Result<Sha256, Bolt11ParseError> {
        if field_data.len() != 52 {
            ...
            Ok(Sha256(
                sha256::Hash::from_slice(&Vec::<u8>::from_base32(field_data)?)
        ...
        match version.to_u8() {
            ...
            17 => {
                let pkh = match PubkeyHash::from_slice(&bytes) {
                    Ok(pkh) => pkh,
                    Err(bitcoin::hashes::Error::InvalidLength(_, _)) => {
                        return Err(Bolt11ParseError::InvalidPubKeyHashLength)
                    }
                };
            ...
            18 => {
                let sh = match ScriptHash::from_slice(&bytes) {
                    Ok(sh) => sh,
                    Err(bitcoin::hashes::Error::InvalidLength(_, _)) => {
                        return Err(Bolt11ParseError::InvalidScriptHashLength)
                    }
                };
...
mod test {
    ...
    fn test_parse_currency_prefix() {
        ...
        assert_eq!(
            "something_else".parse::<Currency>(),
            Err(Bolt11ParseError::UnknownCurrency)
        )
    }
    ...
    fn test_parse_int_from_bytes_be() {
        ...
        assert_eq!(
            parse_u16_be(&[
                ...
                u5::try_from_u8(3).unwrap(),
                u5::try_from_u8(4).unwrap()
            ]),
            Some(34916)
        );
        assert_eq!(
    ...
    fn test_parse_sha256_hash() {
        ...
        assert_eq!(
            Sha256::from_base32(&input_unexpected_length),
            Err(Bolt11ParseError::Skip)
        );
    }
    ...
    fn test_parse_payee_pub_key() {
        ...
        assert_eq!(
            PayeePubKey::from_base32(&input_unexpected_length),
            Err(Bolt11ParseError::Skip)
        );
    }
    ...
    fn test_parse_route() {
        ...
        assert_eq!(
            PrivateRoute::from_base32(&input),
            Ok(PrivateRoute(RouteHint(expected)))
        );
