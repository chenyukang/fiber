    ...
    InvoiceBuilder<D, H, T, C, S, M, X>
{
    ...
    pub fn payee_pub_key(mut self, pub_key: PublicKey) -> Self {
        self.tagged_fields
            .push(TaggedField::PayeePubKey(PayeePubKey(pub_key)));
        self
    }
    ...
    pub fn expiry_time(mut self, expiry_time: Duration) -> Self {
        self.tagged_fields
            .push(TaggedField::ExpiryTime(ExpiryTime::from_duration(
                expiry_time,
            )));
        self
    }
    ...
    InvoiceBuilder<D, H, T, C, tb::False, M, X>
{
    ...
    pub fn payment_secret(
        ...
        self.tagged_fields
            .push(TaggedField::PaymentSecret(payment_secret));
        if !found_features {
    ...
    InvoiceBuilder<tb::True, tb::True, tb::True, tb::True, tb::True, M, X>
{
        ...
        F: FnOnce(&Message) -> Result<RecoverableSignature, E>,
    {
        ...
        invoice
            .check_field_counts()
            .expect("should be ensured by type signature of builder");
        invoice
            .check_feature_bits()
            .expect("should be ensured by type signature of builder");
        invoice
...
impl SignedRawBolt11Invoice {
    ...
    pub fn recover_payee_pub_key(&self) -> Result<PayeePubKey, secp256k1::Error> {
        ...
        Ok(PayeePubKey(
            Secp256k1::new().recover_ecdsa(&hash, &self.signature)?,
        ))
    }
...
impl RawBolt11Invoice {
    ...
    pub fn signable_hash(&self) -> [u8; 32] {
        ...
        RawBolt11Invoice::hash_from_parts(self.hrp.to_string().as_bytes(), &self.data.to_base32())
    }
    ...
    pub fn payment_hash(&self) -> Option<&Sha256> {
        find_extract!(
            self.known_tagged_fields(),
            TaggedField::PaymentHash(ref x),
            x
        )
    }
    ...
    pub fn description_hash(&self) -> Option<&Sha256> {
        find_extract!(
            self.known_tagged_fields(),
            TaggedField::DescriptionHash(ref x),
            x
        )
    }
    ...
    pub fn min_final_cltv_expiry_delta(&self) -> Option<&MinFinalCltvExpiryDelta> {
        find_extract!(
            self.known_tagged_fields(),
            TaggedField::MinFinalCltvExpiryDelta(ref x),
            x
        )
    }
    ...
    pub fn payment_secret(&self) -> Option<&PaymentSecret> {
        find_extract!(
            self.known_tagged_fields(),
            TaggedField::PaymentSecret(ref x),
            x
        )
    }
    ...
    pub fn payment_metadata(&self) -> Option<&Vec<u8>> {
        find_extract!(
            self.known_tagged_fields(),
            TaggedField::PaymentMetadata(ref x),
            x
        )
    }
...
impl Bolt11Invoice {
    ...
    pub fn payment_secret(&self) -> &PaymentSecret {
        self.signed_invoice
            .payment_secret()
            .expect("was checked by constructor")
    }
        ...
        self.fallbacks()
            ...
            .filter_map(|fallback| {
                let payload = match fallback {
                    ...
                    Fallback::PubKeyHash(pkh) => Payload::PubkeyHash(*pkh),
                    Fallback::ScriptHash(sh) => Payload::ScriptHash(*sh),
                };
...
mod test {
    ...
    fn test_check_signature() {
        ...
        assert_eq!(
            ...
            Ok(crate::PayeePubKey(public_key))
        );
    ...
    fn test_check_feature_bits() {
        ...
        let invoice = {
            ...
            invoice
                .data
                .tagged_fields
                .push(PaymentSecret(payment_secret).into());
            invoice.sign::<_, ()>(|hash| {
        ...
        let invoice = {
            ...
            invoice
                .data
                .tagged_fields
                .push(Features(Bolt11InvoiceFeatures::empty()).into());
            invoice.sign::<_, ()>(|hash| {
                Ok(Secp256k1::new().sign_ecdsa_recoverable(hash, &private_key))
            })
        }
        ...
        let invoice = {
            ...
            invoice
                .data
                .tagged_fields
                .push(Features(payment_secret_features.clone()).into());
            invoice.sign::<_, ()>(|hash| {
                Ok(Secp256k1::new().sign_ecdsa_recoverable(hash, &private_key))
            })
        }
        ...
        let invoice = {
            ...
            invoice
                .data
                .tagged_fields
                .push(Features(payment_secret_features).into());
            invoice.sign::<_, ()>(|hash| {
                Ok(Secp256k1::new().sign_ecdsa_recoverable(hash, &private_key))
            })
        }
    ...
    fn test_builder_fail() {
        ...
        assert_eq!(
            sign_error_res,
            Err(SignOrCreationError::SignError("ImaginaryError"))
        );
    }
    ...
    fn test_builder_ok() {
        ...
        let builder = InvoiceBuilder::new(Currency::BitcoinTestnet)
            ...
            .min_final_cltv_expiry_delta(144)
            .fallback(Fallback::PubKeyHash(
                PubkeyHash::from_slice(&[0; 20]).unwrap(),
            ))
        ...
        assert_eq!(
            ...
            vec![&PrivateRoute(route_1), &PrivateRoute(route_2)]
        );
        assert_eq!(
            invoice.description(),
            Bolt11InvoiceDescription::Hash(&Sha256(
                sha256::Hash::from_slice(&[3; 32][..]).unwrap()
            ))
        );
        assert_eq!(
            invoice.payment_hash(),
            &sha256::Hash::from_slice(&[21; 32][..]).unwrap()
        );
    ...
    fn test_default_values() {
        ...
        assert_eq!(
            invoice.expiry_time(),
            Duration::from_secs(DEFAULT_EXPIRY_TIME)
        );
