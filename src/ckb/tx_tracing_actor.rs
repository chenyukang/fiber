...
impl CkbTxTracingMessage {
    pub fn create_tracer(
        ...
        callback: RpcReplyPort<CkbTxTracingResult>,
    ) -> Self {
        Self::CreateTracer(CkbTxTracer {
            tx_hash,
            confirmations,
            mask,
            callback,
    ...
    pub fn remove_tracers(tx_hash: Hash256) -> Self {
        Self::RemoveTracers(tx_hash)
    }
    ...
    async fn report_tracing_result(
        ...
        match tx_status {
            TxStatus::Committed(block_number, ..) => {
                // Always use the lastest committed block number
