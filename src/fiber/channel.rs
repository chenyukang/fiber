    ...
    S: InvoiceStore + ChannelActorStateStore,
{
    ...
    pub async fn handle_peer_message(
        ...
        match message {
            ...
            FiberChannelMessage::CommitmentSigned(commitment_signed) => {
                eprintln!(
                    "peer: {:?}, handle_commitment_signed_peer_message",
            ...
            FiberChannelMessage::RevokeAndAck(revoke_and_ack) => {
                eprintln!(
                    "peer {:?}, handle_revoke_and_ack_peer_message",
                    state.get_local_peer_id()
                );
                state.tlc_state.debug();
    ...
    async fn apply_remove_tlc_operation(
        ...
        if tlc_info.previous_tlc.is_none() {
            ...
            eprintln!(
                "peer {:?} {:?} register relay remove tlc to previous channel: {:?}",
                state.get_local_peer_id(),
                tlc_info.payment_hash,
                tlc_info.tlc_id
            );
    ...
    pub fn handle_commitment_signed_command(
        ...
        eprintln!(
            "peer {:?} begin to handle commitment_signed command, waiting_ack: {:?}",
            state.get_local_peer_id(),
        ...
        eprintln!(
            "peer: {:?}, handle_commitment_signed_command: commitment_tx_partial_signature: {:?}",
            state.get_local_peer_id(),
    ...
    pub fn handle_add_tlc_command(
        ...
        eprintln!(
            "peer {:?} begin to handle add tlc command: {:?}",
    ...
    pub fn handle_remove_tlc_command(
        ...
        eprintln!(
            "peer {:?} begin to handle remove tlc command: {:?}",
            state.get_local_peer_id(),
            &command
        );
        ...
        pending_tlc_ops.retain_mut(|retryable_operation| {
            match retryable_operation {
                RetryableTlcOperation::RemoveTlc(tlc_id, ref reason) => {
                    eprintln!(
                        "peer {:?} payment hash : {:?} here to remove_tlc command",
                        state.get_local_peer_id(),
                        tlc_id
                    );
                ...
                RetryableTlcOperation::RelayRemoveTlc(channel_id, tlc_id, ref reason) => {
                    ...
                    if tlc_info.is_none_or(|tlc| tlc.removed_reason.is_some()) {
                        ...
                        eprintln!(
                            "peer {:?} payment hash : {:?} begin relay remove tlc prev_reestablishing: {:?}  self.state: {:?}",
                            peer,
                            tlc_info.unwrap().payment_hash,
                            prev_channel_state.reestablishing,
                        ...
                        eprintln!(
                            "peer {:?} payment hash : {:?} relay remove tried to send, keep retrying",
                            peer,
                            tlc_info.unwrap().payment_hash
                        );
                        true
                ...
                RetryableTlcOperation::ForwardTlc(
                    ...
                    if !*try_one_time {
                        ...
                        let peer= state.get_local_peer_id();
                        eprintln!("peer {:?} payment_hash : {:?} try to forward tlc", peer, payment_hash);
    ...
    async fn handle_forward_tlc_result(
        ...
        if let Some((tlc_op, peeled_onion)) = pending_ops.iter().find_map(|op| match op {
            ...
            if let Some((channel_err, tlc_err)) = result.error_info {
                eprintln!(
                    "peer {:?} payment_hash {:?} got forward result: {:?}",
                    state.get_local_peer_id(),
                    result.payment_hash,
                    channel_err
                );
                ...
                eprintln!(
                    "peer {:?} payment_hash {:?} finished forward result",
                    state.get_local_peer_id(),
                    result.payment_hash,
                );
    ...
    pub async fn handle_event(
        ...
        match event {
            ...
            ChannelEvent::Stop(reason) => {
                ...
                if reason == StopReason::PeerDisConnected {
                    state.remove_uncommitted_htlcs_and_mark_paused();
                }
                myself.stop(None);
            }
    ...
    S: ChannelActorStateStore + InvoiceStore + Send + Sync + 'static,
{
    ...
    async fn pre_start(
        ...
        match args {
            ...
            ChannelInitializationParameter::ReestablishChannel(channel_id) => {
                ...
                eprintln!(
                    "peer : {:?} now set started, set reestablishing to true",
    ...
    async fn handle(
        ...
        match message {
            ...
            ChannelActorMessage::Command(command) => {
                let command_str = format!("ChannelActorMessage::Command {}", command);
                if let Err(err) = self.handle_command(&myself, state, command).await {
                    error!(
                        "{:?} Error while processing channel command: {:?} command: {:?}",
                        state.get_local_peer_id(),
                        err,
                        command_str,
                    );
    ...
    async fn post_start(
        ...
        if state.tlc_state.has_pending_operations() && !state.reestablishing {
            eprintln!(
                "now peer {:?} has pending tlc operations, so we need to check them",
                state.get_local_peer_id()
            );
            myself
                .send_message(ChannelActorMessage::Event(
...
impl TlcInfo {
    pub fn log(&self) -> String {
        format!(
            "id: {:?} status: {:?} amount: {:?} remove_reason: {:?} payment_hash: {:?}",
...
impl TlcState {
    ...
    pub fn set_offered_tlc_removed(&mut self, tlc_id: u64, reason: RemoveTlcReason) -> Hash256 {
        ...
        eprintln!("set_offered_tlc_removed: {:?}", tlc.log());
...
pub struct ChannelActorState {
    ...
    pub resend_order: RAACommitmentOrder,
}

/// channel reestablish needs to re-send messages in the order
...
impl ChannelActorState {
    ...
    fn remove_uncommitted_htlcs_and_mark_paused(&mut self) {
        ...
        for tlc in self.tlc_state.received_tlcs.tlcs.iter_mut() {
            if tlc.status == TlcStatus::Inbound(InboundTlcStatus::AnnounceWaitPrevAck) {
                tlc.status = TlcStatus::Inbound(InboundTlcStatus::AnnounceWaitAck);
            }
        }
        self.clear_waiting_peer_response();
    ...
    fn send_revoke_and_ack_message(&mut self) -> ProcessingChannelResult {
        ...
        let commitment_tx_partial_signature = {
            ...
            eprintln!("send_revoke_and_ack_message, message: {:?}", message);
        ...
        eprintln!(
            "peer {:?} send_revoke_and_ack_message, commitment_tx_partial_signature: {:?},",
            self.get_local_peer_id(),
            commitment_tx_partial_signature
        );
    ...
    pub fn remove_tlc_with_reason(
        ...
        if let RemoveTlcReason::RemoveTlcFulfill(fulfill) = &reason {
            ...
            if current.payment_hash != filled_payment_hash {
                eprintln!(
                    "TLC payment hash {:?} does not match the preimage {:?}",
                    current.payment_hash, filled_payment_hash
                );
                return Err(ProcessingChannelError::FinalIncorrectPreimage);
            }
        ...
        eprintln!(
            "peer {:?} Removed tlc payment_hash {:?} with reason {:?}",
            self.get_local_peer_id(),
    ...
    async fn on_reestablished_channel_ready(&mut self, myself: &ActorRef<ChannelActorMessage>) {
        ...
        self.on_owned_channel_updated(myself, false).await;
        eprintln!(
            "\n\npeer {:?} finished reestablish ....\n\n",
            self.get_local_peer_id()
        );
    }

    fn mark_retry_ops_for_reestablish(&mut self) {
        let peer = self.get_local_peer_id();
        for op in self.tlc_state.retryable_tlc_operations.iter_mut() {
            match op {
                RetryableTlcOperation::ForwardTlc(payment_hash, _, _, _, try_one_time) => {
                    eprintln!(
                        "peer {:?} payment_hash : {:?} mark retry ops for reestablish",
                        peer, payment_hash
                    );
                    *try_one_time = true;
                }
    ...
    fn handle_revoke_and_ack_peer_message(
        ...
        let revocation_data = {
            ...
            if aggregated_signature.is_err() {
                eprintln!(
                    "is error now ...............: {:?} message: {:?}",
                    aggregated_signature, message
                );
            }
            let aggregated_signature = aggregated_signature?;
            RevocationData {
                commitment_number,
                x_only_aggregated_pubkey,
                aggregated_signature,
    ...
    async fn handle_reestablish_channel_message(
        ...
        match self.state {
            ...
            ChannelState::ChannelReady => {
                let waiting_ack = self.tlc_state.waiting_ack;
                eprintln!(
                    "peer {:?} reestablish channel in ChannelReady state, wait_ack: {:?}",
                    self.get_local_peer_id(),
                    waiting_ack,
                );
                ...
                eprintln!("peer: {:?} expected_local: {:} expected_remote: {:} real_local: {:} real_remote: {:}",
                    self.get_local_peer_id(),
                    expected_local_commitment_number,
                    expected_remote_commitment_number,
                    actual_local_commitment_number,
                    ...
                    && expected_remote_commitment_number == actual_remote_commitment_number
                {
                    ...
                    let tlcs: Vec<_> = self.tlc_state.all_tlcs().map(|t| t.clone()).collect();
                    for info in tlcs {
                        if info.is_offered()
                    ...
                    && expected_local_commitment_number == actual_local_commitment_number
                {
                    ...
                    if let Some(last_revoke_ack_msg) = self.last_revoke_ack_msg.clone() {
                        eprintln!(
                            "peer {:?} reuse resend RevokeAndAck message: {:?}",
                            self.get_local_peer_id(),
                            last_revoke_ack_msg
                        );
                        self.network()
    ...
    fn build_commitment_transaction_output(&self, for_remote: bool) -> (CellOutput, Bytes) {
        ...
        //let tlcs_hash = blake2b_256(&tlcs);
        //eprintln!("build commitment transaction tlcs_hash: {:?}", tlcs_hash);
    ...
    fn build_settlement_transaction_outputs(
        ...
        let pending_tlcs = self
            ...
            .chain(self.tlc_state.received_tlcs.tlcs.iter().filter(move |tlc| {
                match tlc.inbound_status() {
                    InboundTlcStatus::RemoteAnnounced => !for_remote && !self.reestablishing,
                    InboundTlcStatus::AnnounceWaitPrevAck => !for_remote && !self.reestablishing,
                    InboundTlcStatus::AnnounceWaitAck => true,
                    InboundTlcStatus::Committed => true,
