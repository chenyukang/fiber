...
#[tokio::test]
async fn test_network_send_expired_invoice() {
    init_tracing();
    ...
    let node_b_invoice = InvoiceBuilder::new(crate::invoice::Currency::Fibd)
        .expiry_time(Duration::from_secs(1))
        .payee_pub_key(node_b_pubkey.into())
        .amount(Some(10000))
        .build()
        .unwrap();
    ...
    let message = |rpc_reply| -> NetworkActorMessage {
        NetworkActorMessage::Command(NetworkActorCommand::SendPayment(
            SendPaymentCommand {
                ...
                payment_hash: None,
                final_htlc_expiry_delta: None,
                invoice: Some(node_b_invoice.to_string()),
                timeout: None,
                max_fee_amount: None,
                max_parts: None,
                keysend: None,
    ...
    // sleep 2 seconds to make sure the previous tlc is expired
    tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;

    let res = call!(node_a.network_actor, message).expect("node_a alive");
    assert!(res.err().unwrap().contains("invoice is expired"));
}
            ...
            ChannelCommandWithId {
                ...
                command: ChannelCommand::AddTlc(
                    AddTlcCommand {
                        ...
                        expiry: DEFAULT_EXPIRY_DELTA + now_timestamp(),
