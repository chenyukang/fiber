...
async fn test_network_send_payment_more_send_each_other() {
    ...
    assert!(matches!(
        res2.status,
        PaymentSessionStatus::Inflight | PaymentSessionStatus::Created
    ));
    let payment_hash2 = res2.payment_hash;
...
async fn test_network_add_two_tlcs_remove_one() {
    ...
    // if we don't wait for a while, the next add_tlc will also ok but it's pending on the TlcState
    let preimage_b = [2; 32];
    let algorithm = HashAlgorithm::Sha256;
    ...
    .expect("node_b alive");
    eprintln!("add_tlc_result_b: {:?}", add_tlc_result_b);
    assert!(add_tlc_result_b.is_ok());

    // now wait for a while, then add a tlc again, it will be error with duplicate payment_hash
    tokio::time::sleep(tokio::time::Duration::from_millis(200)).await;
    let add_tlc_result_c = call!(node_a.network_actor, |rpc_reply| {
        NetworkActorMessage::Command(NetworkActorCommand::ControlFiberChannel(
            ChannelCommandWithId {
                channel_id: channel_id,
                command: ChannelCommand::AddTlc(
    ...
    eprintln!("add_tlc_result_c: {:?}", add_tlc_result_c);
    assert!(add_tlc_result_c.is_err());

    tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
    // remove the first tlc from node_b
