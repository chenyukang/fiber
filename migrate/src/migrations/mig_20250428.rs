...
impl Migration for MigrationObj {
    fn migrate(
            ...
            .take_while(|(col_key, _)| col_key.starts_with(prefix.as_slice()))
        {
            ...
            let payment_session_v020 = bincode::deserialize::<OldPaymentSessionV020>(&v);
            if let Ok(payment_session_v020) = payment_session_v020 {
                let new_payment_session = migrate_payment_session_v020(payment_session_v020);
                let new_payment_session_bytes = bincode::serialize(&new_payment_session)
                    .expect("serialize to new payment session");
            ...
            let payment_session_v050 = bincode::deserialize::<OldPaymentSessionV050>(&v);
            if let Ok(payment_session_v050) = payment_session_v050 {
                let new_payment_session = migrate_payment_session_v050(payment_session_v050);
                let new_payment_session_bytes = bincode::serialize(&new_payment_session)
                    .expect("serialize to new payment session");
